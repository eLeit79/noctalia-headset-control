...
TRAPS.md               host/toolchain traps already walked into (repo-only)
...
Moved to **[TRAPS.md](TRAPS.md)** — host and toolchain behaviour that has already cost
time once: invisible `mOnPrimary` text, `ln -sfn` over a real directory, `head -1` under
`pipefail`, Quickshell's silence when a command cannot start, SIGTERM misread as a
rejected value, `closePanel(undefined)` closing someone else's panel, and why every
`headsetcontrol` call is checked. Read it before changing behaviour.
